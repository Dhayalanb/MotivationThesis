...
bin_dir=../bin/
cd ${name}
export CC=../${bin_dir}/angora-clang
ANGORA_USE_ASAN=1 USE_FAST=1  make -j 4 
mv jhead jhead.fast 
make clean

USE_TRACK=1 make -j 4 
mv jhead jhead.taint 
make clean

export CC=../${bin_dir}/angora-clang-oracle
make -j 4 
mv jhead jhead.orc
make clean

export CC=../${bin_dir}/static-analysis-clang 
make -j 4 
mv jhead jhead.static
make clean

export CC=../${bin_dir}/symcc
make -j 4 
mv jhead jhead.sym
make clean
