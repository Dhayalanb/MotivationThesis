...
bool StaticMetrics::runOnModule(Module &M) {
  ...
  if (!Output) {
    Output =
        make_unique<std::ofstream>(std::string("./static_analysis_results.") + std::to_string(ModId) + std::string(".out"), std::ios::out);
...
void StaticMetrics::print(raw_ostream &O, Module const *) const {
  O << "Metrics have been stored into '" << "./static_analysis_results." << std::to_string(ModId).c_str() << ".out" << "'\n";
}
